...
| Tier | What it is | How | Agents |
|---|---|---|---|
| **0 — Universal** | `AGENTS.md` + file artifacts + the `bevel` CLI | Nothing agent-specific | Any |
| **1 — Commands** | `/shape`, `/implement` invocable | Rendered per adapter from one source | Claude Code, Codex, Cursor, Gemini, opencode |
| **2 — Orchestration** | Subagents, blocking hooks, worktrees, workflows | Claude Code only | Claude Code |
...
into `.claude/skills/`, `.claude/agents/`, `.claude/settings.json`, `AGENTS.md`. On Linux it
uses **symlinks** where possible (updating the harness updates every project at once) and
generated files with a `<!-- generated by harness, do not edit -->` header where the format
forbids symlinks.
...
Full `AGENTS.md`, Codex/Cursor adapters, `doctor --context`, migrations, model routing, hooks.
